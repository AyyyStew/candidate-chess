    ...
    for node in puzzle.mainline[1::2][1:]:
        ...
        if capture and util.is_hanging(prev_board, capture, square):
            ...
            if (
                ...
                if (
                    node.parent.move
                    and node.parent.move.to_square
                    in SquareSet.between(square, defender)
                ):
                    return True
    return False
...
def collinear(puzzle: Puzzle) -> bool:
    ...
    for node in puzzle.mainline[1::2]:
        ...
        for square in prev_board.attacks(from_sq):
            ...
            is_orthogonal = square_rank(from_sq) == square_rank(square) or square_file(
                from_sq
            ) == square_file(square)
