import type { StoredCandidate } from "../services/dailyStatsService";
...
export default function DailyResultsPanel({
  ...
  const misses = record.candidates.filter((c) => !c.isHit);

  // Build a map from rank → candidate that found it
  const foundByRank = new Map<number, StoredCandidate>();
  for (const c of record.candidates) {
    if (c.isHit && c.rank != null) foundByRank.set(c.rank, c);
  }
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-baseline gap-2">
          ...
          <span className="text-base text-muted">/ {record.target} moves found</span>
        </div>
        <div className="flex gap-1.5 text-xl">
          {record.squares.map((s, i) => <span key={i}>{s}</span>)}
          {Array.from({ length: Math.max(0, record.target - record.squares.length) }).map((_, i) => (
            <span key={`e-${i}`}>⬜</span>
          ))}
        </div>
      </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-col gap-1.5">
            {record.answers.map((answer, i) => (
              <MoveRowCard
                key={i}
                rank={answer.rank ?? i + 1}
                san={answer.san}
                category={answer.category}
                eval={answer.eval}
                isHit={foundByRank.has(answer.rank!) ? true : false}
              />
            ))}
          </div>
        </div>
